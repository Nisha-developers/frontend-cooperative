...
const BuyApartment = () => {
 ...
 const {previewApartment, preview, setPreview, applyPurchase,  getPurchase, } = usePurchase();
 const {PreviewRent, previewValue,  title, message, open, type, ApplyRent, setOpen} = useRent();
...
const submitForm = (data) =>{
  setDurationDays(data.duration_days)
   ...
   PreviewRent({duration_days: data.duration_days,listing_id: apartmentId})
   setFormOpen(false)
   setIsOpen(true);
   ...
   <div className="min-h[90vh]bg-cooperative-cream flex items-center justify-center px-4 small-screen:px-6">
      <div className="max-w-md w-full text-center">
        ...
        <p className="text-cooperative-dark/70 text-base md:text-lg mb-8">
          ...
          Explore our available apartments and find your perfect home. Click the link below
        </p>
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between max-sm:flex-col max-sm:items-start gap-4">
        <div>
          ...
          <p className="text-cooperative-dark/70 mt-2">Complete the payment to get an apartment according to what you have chosen to rent</p>
        </div>
      </div>
            ...
            <div className="flex-1 p-5 flex flex-col justify-between">
              ...
              <div className="grid grid-cols-3 gap-2 mb-4">
                <div className="bg-cooperative-cream rounded-xl p-3">
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-cooperative-dark/40 mb-1">Price per day</p>
    ...
    <div className="w-full max-w-md rounded-2xl bg-cooperative-cream overflow-hidden shadow-2xl">
      ...
      <div className="px-6 py-6 space-y-5">
        ...
        <div className="flex items-center justify-between rounded-lg bg-cooperative-dark/5 px-5 py-3">
          <span className="text-xs uppercase tracking-widest text-cooperative-dark/70 font-semibold">Listing</span>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-lg bg-cooperative-teal/5 p-4 border border-cooperative-teal/20">
            <p className="text-[10px] uppercase tracking-widest font-bold text-cooperative-teal mb-2">Price Per Day</p>
            <p className="text-lg font-bold text-cooperative-dark">
              ₦{previewValue?.price_per_day?.toLocaleString() || 0}
            </p>
          </div>
          <div className="rounded-lg bg-cooperative-orange/5 p-4 border border-cooperative-orange/20">
            <p className="text-[10px] uppercase tracking-widest font-bold text-cooperative-orange mb-2">Duration</p>
            <p className="text-lg font-bold text-cooperative-dark">
              {previewValue?.duration_days} {previewValue?.duration_days === 1 ? 'Day' : 'Days'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between rounded-lg bg-cooperative-teal px-5 py-4">
          <span className="text-xs uppercase tracking-widest text-white font-semibold">Total Rent Cost</span>
          <span className="text-xl font-bold text-cooperative-cream">
            ₦{previewValue?.total_rent_cost?.toLocaleString()}
          </span>
        </div>
...
{createPortal(<PopupMessage isOpen={open} title={title} message={message} type={type} onClose={()=>setOpen(false)}/>,document.body)}
    {createPortal(<PopupForm isOpen={formOpen} onClose={()=>setFormOpen(false)} formfield={duration} onSubmit={(data)=>submitForm(data)}  title='Duration' submitLabel='Submit duration'  />, document.body)}
    </div>
  );
};
